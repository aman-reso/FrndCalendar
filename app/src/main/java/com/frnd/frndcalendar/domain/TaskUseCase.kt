...
import com.frnd.frndcalendar.utility.universalApiRequestManager
import javax.inject.Inject

class TaskUseCase @Inject constructor(
    private val remoteRepository: TaskRemoteRepository, private val taskRepository: TaskLocalRepository,
) {
    suspend fun storeCalendarTask(request: StoreTaskRequest) = universalApiRequestManager {
        remoteRepository.storeCalendarTask(request)
    }

    suspend fun deleteCalendarTask(request: DeleteTaskRequest) = universalApiRequestManager {
        remoteRepository.deleteCalendarTask(request)
    }

    suspend fun getCalendarTaskList(request: GetTasksRequest) = universalApiRequestManager {
        remoteRepository.getCalendarTaskList(request)
    }
